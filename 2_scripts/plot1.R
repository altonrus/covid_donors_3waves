...
library(dplyr)
library(ggplot2)
data_df <- posterior_predict(mod3_atl, ndraws = 1000)
...
######## glm
data_df <- posterior_predict(mod3_atl_glm, ndraws = 1000)
data_df <- data.frame(t(data_df))
data_df$cur_result_n <- dat$cur_result_n
data_df$cduid <- dat$cduid
...
est_by_area_all$model <- c(rep('MLM spatial', nrow(est_by_area)), rep('GLM', nrow(est_by_area)), 
                           rep('Obs', nrow(est_by_area)))
...
desired_order <- c( "GLM", "Obs", "MLM spatial")
...
#  geom_point(data = est_by_area, aes(x=cduid, y=cur_result_n), position=pd_1, colour = "violetred", size = 1) +
  ...
  ggtitle("Atlantic, Pre-Delta") +
  theme(plot.title = element_text(vjust = 0)) + 
  scale_color_manual(values = c("GLM" = "green", "Obs" = "violetred", "MLM spatial" = "blue"),
                     name = "type",
